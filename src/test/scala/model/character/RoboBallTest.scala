...
class RoboBallTest extends munit.FunSuite {
  ...
  test("A Robo Ball wild unit should have correctly set their attributes") {
  ...
  test("A Robo Ball wild unit should have 2 stars at the beginning of the game") {
    assertEquals(enemy.stars, 2)
  }

  test("A Robo Ball wild unit stars can't be less than 2") {
    enemy.stars = -4
    assertEquals(enemy.stars, 2)
  }

  test("A Robo Ball wild unit should be able to increased their stars counter through the game") {
    enemy.stars += 3
    assertEquals(enemy.stars, 5)
  }

  test("A Robo Ball wild unit can lose stars through the game, so it should be able to decreased their stars counter") {
    enemy.stars += 3
    enemy.stars -= 2
